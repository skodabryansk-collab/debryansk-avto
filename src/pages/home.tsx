...
async function fetchDealers(): Promise<ApiDealer[]> {
...
export default function Home() {
  ...
  const { data: apiDealers = [] } = useQuery({
  ...
  const dealerMapLocations = React.useMemo(() => {
    if (apiDealers.length === 0) return [];
    // Group by real coordinates
    const groups = new Map<string, typeof apiDealers[0][]>();
    for (const d of apiDealers) {
      const key = `${d.mapX?.toFixed(4)},${d.mapY?.toFixed(4)}`;
      if (!groups.has(key)) groups.set(key, []);
      groups.get(key)!.push(d);
    }
    const COLORS = ["#0070b8", "#87b63c", "#0070b8", "#87b63c"];
    let idx = 0;
    return Array.from(groups.values()).map(dealersAtLoc => {
      const first = dealersAtLoc[0];
      const allBrands = dealersAtLoc.flatMap(d => d.brands ?? []);
      const uniqueBrands = [...new Set(allBrands)];
      return {
        id: first.id,
        address: first.address,
        short: first.shortName,
        brands: uniqueBrands,
        lat: first.mapX ?? 53.24,
        lng: first.mapY ?? 34.36,
        color: COLORS[idx++ % COLORS.length],
        phone: first.phone ?? undefined,
        hours: first.hours ?? undefined,
      };
    });
  }, [apiDealers]);

  const dealerLocations = React.useMemo(() =>
    dealerMapLocations.map((loc, i) => ({
      id: loc.id,
      address: loc.address,
      short: loc.short,
      brands: loc.brands,
    ...
    <div className="flex flex-col min-h-screen bg-white font-sans text-slate-900">
      ...
      <section className="relative flex items-center justify-center overflow-hidden" style={{ minHeight: "100dvh" }}>
        ...
        <div className="relative z-10 w-full pt-[6rem] pb-8 sm:pt-[5.5rem] sm:pb-0">
          <div className="container mx-auto px-4 sm:px-6 flex flex-col items-center text-center">
            ...
            >
              9 брендов · 4 дилерских центра · Брянск.<br className="hidden sm:block" />{" "}
      ...
      <section id="dealers" className="py-16 sm:py-24 md:py-32 bg-slate-50">
        <div className="container mx-auto px-4 sm:px-6">
          ...
          <div className="w-full h-[400px] sm:h-[500px] md:h-[600px] rounded-2xl overflow-hidden border border-slate-200 shadow-lg">
            <YandexMap locations={dealerMapLocations} />
          </div>
