...
## Восстановление моделей на VPS
**Правило:** Replit-managed production БД и рабочая БД VPS — разные источники. Первую можно проверить как историческую копию настроек CMS, но нельзя считать текущей базой сайта.

**Why:** прежний модельный ряд сохранился в старой Replit production БД, хотя на VPS поле было пустым. Ссылки на старые фотографии без расширения вели к файлам, оставшимся в Replit Object Storage; на VPS они отвечали 404. Снимок страницы для поисковых роботов тоже оставался старым после изменения БД.

**How to apply:** сверить точную конфигурацию нужного бренда и менять только пустое поле `models`, не затрагивая остальной контент. До включения карточек перенести недостающие фотографии на VPS; после восстановления пересобрать только соответствующий брендовый prerender и проверить браузерную и бот-версию страницы.
