    ...
    >
      <div className="relative h-48 bg-slate-100 overflow-hidden">
        {img ? (
          <img
            src={img}
            alt={`${car.mark} ${car.model} ${car.year}${car.complectation ? `, ${car.complectation}` : ""}`}
            loading="lazy"
            decoding="async"
      ...
      <div className="p-4 flex flex-col flex-1">
        <h3 className="font-extrabold text-base leading-snug mb-0.5">
          <Link
            href={`/new-cars/${encodeURIComponent(car.id)}`}
            onClick={e => {
              e.stopPropagation();
              fetch(`/api/cars/views/${encodeURIComponent(car.id)}`, { method: "POST" }).catch(() => {});
            }}
  ...
  const filterSections: FilterSection[] = [
    {
      kind: "pills",
      label: "Наличие",
      options: ["Все", "В наличии", "На заказ"],
      value: filterAvailability,
      onSelect: a => go(() => setFilterAvailability(a)),
      activeClass: "bg-[#87b63c]",
    },
    {
      kind: "pills",
      label: "Дилер / Бренд",
      options: DEALERS,
      value: filterDealer,
      onSelect: d => go(() => { setFilterDealer(d); setFilterModel("Все модели"); }),
    },
    ...(availableModels.length > 2 ? [{
      ...
      value: filterModel,
      onSelect: (m: string) => go(() => setFilterModel(m)),
    }] : []),
    {
      kind: "range",
      label: "Цена, ₽",
