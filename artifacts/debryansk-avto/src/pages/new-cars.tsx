      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        <div className="mt-auto">
          ...
          <button
            ...
            className="w-full bg-gradient-to-r from-primary to-[#005a94] text-white font-bold rounded-xl py-2.5 text-sm hover:opacity-90 transition-opacity flex items-center justify-center gap-2"
          >
        ...
        <div className="flex flex-wrap gap-1.5">
          {DEALERS.map(d => (
            <button key={d} onClick={() => go(() => { setFilterDealer(d); setFilterModel("Все модели"); })}
              className={`px-2.5 py-1 rounded-full text-[11px] font-bold transition-all ${
                filterDealer === d ? "bg-primary text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"
              }`}
            >{d}</button>
          ))}
        </div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {availableModels.map(m => (
              <button key={m} onClick={() => go(() => setFilterModel(m))}
                className={`px-2.5 py-1 rounded-full text-[11px] font-bold transition-all ${
                  filterModel === m ? "bg-primary text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"
                }`}
              >{m}</button>
            ))}
          </div>
    ...
    <Layout>
      ...
      <div className="container mx-auto px-4 sm:px-6 py-6 sm:py-10">
        <div className="flex items-start justify-between mb-5 sm:mb-8 gap-4">
          <div>
            <p className="text-xs font-bold uppercase tracking-widest text-primary mb-1">В наличии</p>
