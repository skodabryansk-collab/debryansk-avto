...
async function fetchReviews(): Promise<{ data: ApiReview[]; avg: number; total: number }> {
  const r = await fetch("/api/reviews");
  if (!r.ok) return { data: [], avg: 5, total: 0 };
  const json = await r.json();
  return json.ok ? { data: json.data ?? [], avg: json.avg ?? 5, total: json.total ?? 0 } : { data: [], avg: 5, total: 0 };
}
...
const HomeReviewsSection = () => {
  ...
  /* Hide block while loading or when no reviews */
