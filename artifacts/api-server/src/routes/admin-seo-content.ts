...
${addressRules}
- Тема про сервис/ТО/ремонт нескольких брендов ИЛИ «выбор сервиса в Брянске» → ПЕРЕЧИСЛИ ВСЕ адреса из факт-листа выше (все 4 точки)
- Тема про сервис/ТО одного конкретного бренда → только адрес этого бренда
- Если бренд не упоминается → ПЕРЕЧИСЛИ ВСЕ адреса, а не один
НЕ ПРИДУМЫВАЙ адреса. Используй только те, что в факт-листе выше.

━━━ ПРИМЕР — КАК НАДО vs КАК НЕЛЬЗЯ ━━━

❌ ТАК НЕЛЬЗЯ (типичная AI-заготовка, запрещена):
«Автосалоны Брянска предлагают множество вариантов, но не все одинаково надёжны. Первоочередная задача — понять, какой сервис и качество вас ждут при покупке. Многие клиенты забывают проверить условия гарантийного обслуживания...»

✅ ТАК НАДО (конкретная ситуация, живой голос):
«Человек выбирает машину месяц. Смотрит фото, читает отзывы, ездит на тест-драйвы. А потом приходит к дилеру — и слышит: "У нас нет этой комплектации, но вот похожая, чуть дороже". Так теряется время и доверие. Разбираемся, как этого не допустить.»

━━━ СТРУКТУРА: 5 АБЗАЦЕВ С КОНКРЕТНЫМИ ЗАДАЧАМИ ━━━

АБЗАЦ 1 (зацепка) — ОБЯЗАТЕЛЬНО начни с одного из:
  • Конкретная ситуация от первого или третьего лица
  • Неожиданный факт, цифра или парадокс по теме
  • Вопрос, который задаёт сам покупатель перед покупкой
  НЕ начинай со слов: «Сегодня», «В Брянске», «Автосалоны», «Покупка автомобиля», «Выбор машины», «Многие»

АБЗАЦ 2 — один конкретный аргумент / совет, раскрытый через пример или механизм.
  Если уместно — назови конкретную модель из портфеля.

АБЗАЦ 3 — второй аргумент, другой по теме.
  Не повторяй ключевые слова из абзаца 2.

АБЗАЦ 4 — третий аргумент или уточнение (40–60 слов достаточно).
  Здесь органично упомяни «Дебрянск Авто» — один раз, без пафоса.

АБЗАЦ 5 (финал) — конкретный адрес + конкретное действие:
  Что можно сделать: приехать на тест-драйв / записаться на ТО / посмотреть авто с пробегом.
  НЕ пиши: «обращайтесь», «не упустите», «надёжный партнёр», «сделайте правильный выбор»,
  «полный спектр услуг», «уютная атмосфера», «команда профессионалов»

━━━ СТОП-СЛОВА (нарушение = плохая статья) ━━━
Запрещены дословно и по смыслу:
  множество вариантов · широкий выбор · выгодные условия · уютная атмосфера
  команда профессионалов · надёжный партнёр · полный спектр услуг
  Сегодня всё больше людей · Не секрет, что · Актуальность данной темы
  Также важно отметить · Кроме того · При этом · Именно поэтому · Следует отметить
  В заключение хочется отметить · Таким образом · Подводя итоги
  данный · осуществить · в рамках · на сегодняшний день · сотрудничать на долгосрочной основе
  первоочередная задача · обратите внимание (как вводное) · признаки профессионализма

Запрещено: цены, ставки по кредиту, количество авто, бренды НЕ из портфеля.
...
router.post("/generate-article", async (req, res) => {
  ...
  // Строим точный список адресов для инъекции прямо в user-промт
  const addressBlock = locationFacts.length > 0
    ? locationFacts.map(l => {
        const brands = [...l.salesBrands, ...l.serviceBrands.map(b => `${b} (сервис)`)];
        const usedTag = l.hasUsedCars ? " | авто с пробегом" : "";
        return `• ${l.address} — ${brands.join(", ")}${usedTag}`;
      }).join("\n")
  ...
  const prompt =
    ...
    `ТОЧНЫЕ АДРЕСА ДЕБРЯНСК АВТО — используй ТОЛЬКО их, дословно, без изменений:\n` +
    `${addressBlock}\n` +
    `Любой другой адрес — это галлюцинация. Проверь финальный абзац перед ответом.\n\n` +
    ...
    `  "content": "5 абзацев через \\n\\n. Первый — зацепка. Три средних — суть. Пятый — приглашение с адресом(ами) Дебрянск Авто из списка выше. Каждый абзац 60–100 слов.",\n` +
