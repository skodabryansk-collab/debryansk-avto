...
async function resolveLocationEmail(body: Record<string, string>): Promise<string | null> {
  try {
    // Check location field (used by service forms)
    if (body.location) {
      const r = await db.execute(sql`
        SELECT email FROM locations
        WHERE LOWER(TRIM(title)) = LOWER(TRIM(${body.location}))
          AND email IS NOT NULL AND email <> ''
        LIMIT 1
      `);
      const e = (r.rows[0] as { email?: string } | undefined)?.email;
      if (e) return e;
    }
    let dealer = body.dealer;
    if (dealer) {
      const key = dealer.trim().toLowerCase();
      if (DEALER_ALIASES[key]) {
        dealer = DEALER_ALIASES[key];
      }
      const r = await db.execute(sql`
        SELECT email FROM locations
        WHERE LOWER(TRIM(title)) = LOWER(TRIM(${dealer}))
          AND email IS NOT NULL AND email <> ''
        LIMIT 1
      `);
      const e = (r.rows[0] as { email?: string } | undefined)?.email;
    ...
    const brandName = body.brand || body.carMark;
    if (brandName) {
      const r = await db.execute(sql`
        SELECT l.email FROM locations l
        JOIN location_brands lb ON lb.location_id = l.id
        JOIN brands b ON b.id = lb.brand_id
        WHERE LOWER(TRIM(b.name)) = LOWER(TRIM(${brandName}))
...
function buildLeadHtml(d: Record<string, string>) {
  const carTitle = d.carMark && d.carModel
    ? `${d.carMark} ${d.carModel}${d.carYear ? ", " + d.carYear : ""}`
    : (d.car || "");
  return wrapEmail(
    banner("🚗", "Заявка на автомобиль", BLUE) +
    heading("Новая заявка на авто", "Клиент оставил заявку через кнопку «Оставить заявку» на карточке автомобиля") +
    carCard(d.carMark || "", d.carModel || "", d.carYear || "", "") +
    dataTable([
      ["Имя клиента",     d.name],
      ["Телефон",         d.phone],
      ["Автомобиль",      carTitle || undefined],
      ["Дата / время",    new Date().toLocaleString("ru-RU")],
    ]) +
    hr() +
    actionBlock(d.phone, undefined, "Связаться с клиентом", BLUE),
    BLUE
  );
}
...
function buildTradeInHtml(d: Record<string, string>) {
  ...
  return wrapEmail(
    ...
    dataTable([
      ...
      ["Тип кузова",         d.bodyName],
      ["Объём двигателя",    d.engineVolume],
      ["Тип привода",        d.drive],
      ["Мощность двигателя", d.power],
      ["Тип КПП",            d.gear],
      ["Кол-во дверей",      d.doors],
...
function buildPromoHtml(d: Record<string, string>) {
  const accent = "#87b63c";
  return wrapEmail(
    banner("🎁", "Заявка по акции", accent) +
    heading("Клиент интересуется акцией", d.source || "Заявка оставлена через страницу акции на сайте") +
    dataTable([
      ["Телефон", d.phone],
      ["Источник", d.source],
    ]) +
    hr() +
    actionBlock(d.phone, undefined, "Связаться с клиентом по акции", accent),
    accent
  );
}

function buildCorporateHtml(d: Record<string, string>) {
  const accent = "#0070b8";
  return wrapEmail(
    banner("🏢", "Корпоративная заявка", accent) +
    heading("Заявка от корпоративного клиента", "Заявка оставлена через страницу /corporate") +
    dataTable([
      ...
      ["Телефон",          d.phone],
      ["Email",            d.email],
      ["Кол-во автомобилей", d.carCount],
      ["Интересующие бренды", d.brands],
      ["Комментарий",      d.comment],
    ]) +
    hr() +
    actionBlock(d.phone, d.email, "Связаться с клиентом", accent),
    accent
  );
}
...
function buildToCalculatorHtml(d: Record<string, string>) {
  const accent = "#0070b8";
  const GREEN = "#87b63c";
  const totalStr = d.totalSum ? Number(d.totalSum).toLocaleString("ru-RU") + " ₽" : undefined;
  const servicesStr = d.sumServices ? Number(d.sumServices).toLocaleString("ru-RU") + " ₽" : undefined;
  const partsStr = d.sumSpareParts ? Number(d.sumSpareParts).toLocaleString("ru-RU") + " ₽" : undefined;
  const mileageStr = d.mileage ? Number(d.mileage).toLocaleString("ru-RU") + " км" : undefined;
  const resultBlock = (totalStr || servicesStr || partsStr) ? `
  <div style="margin:14px 28px 0;background:linear-gradient(135deg,#f0f7ff,#e8f3fc);border:1px solid ${accent}22;border-radius:12px;padding:16px 20px;font-family:Arial,sans-serif">
    <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.5px;color:#64748b;margin-bottom:10px">Результат расчёта</div>
    <table cellpadding="0" cellspacing="0" style="width:100%"><tr>
      ${servicesStr ? `<td style="padding-right:8px"><div style="background:#fff;border-radius:8px;padding:10px 14px;border:1px solid #e2e8f0"><div style="font-size:10px;color:#64748b;margin-bottom:3px">Работы</div><div style="font-weight:800;color:#1a2332;font-size:14px">${servicesStr}</div></div></td>` : ""}
      ${partsStr ? `<td style="padding-right:8px"><div style="background:#fff;border-radius:8px;padding:10px 14px;border:1px solid #e2e8f0"><div style="font-size:10px;color:#64748b;margin-bottom:3px">Запчасти</div><div style="font-weight:800;color:#1a2332;font-size:14px">${partsStr}</div></div></td>` : ""}
      ${totalStr ? `<td><div style="background:${accent};border-radius:8px;padding:10px 14px"><div style="font-size:10px;color:#bfdbfe;margin-bottom:3px">Итого</div><div style="font-weight:800;color:#fff;font-size:16px">${totalStr}</div></div></td>` : ""}
    </tr></table>
  </div>` : "";
  return wrapEmail(
    banner("🔧", "Запись на ТО — Калькулятор стоимости", accent) +
    heading("Клиент хочет записаться на техническое обслуживание", "Заявка оставлена через калькулятор ТО на брендовой странице") +
    dataTable([
      ["Имя клиента",   d.name],
      ["Телефон",       d.phone],
      ["Бренд",         d.brand],
      ["Модель",        d.model],
      ["Модификация",   d.maintenance],
      ["Вид ТО",        d.toType],
      ["Пробег ТО",     mileageStr],
      ["Год выпуска",   d.year],
    ]) +
    resultBlock +
    hr() +
    actionBlock(d.phone, undefined, "Записать клиента на ТО", accent),
    accent
  );
}
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const locationEmail = await resolveLocationEmail(body);
      const mailTo  = locationEmail ?? TO;
      const mailCc  = locationEmail ? [TO] : [];
      ...
      for (let attempt = 1; attempt <= 2; attempt++) {
        try {
          ...
          console.error(`[email] lead save error (attempt ${attempt}/2):`, dbErr);
